...
flowchart TD
  ...
  presStore["presentation store<br/>filesystem today; SQL later"]
...
- IIIF Content Search 2.0 (`search.txt`) — `Searcher` interface, default no-op backend, and `GET|HEAD /search/v2/{itemID}/search?q=...` HTTP surface now exist. Solr/OpenSearch/database adapters remain follow-up work. Triplet does not own indexing.
