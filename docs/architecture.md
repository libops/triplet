...
flowchart TD
  ...
  searcher["Searcher interface<br/>default no-op; adapters later"]
  authorizer["Authorizer interface<br/>default permit-all; adapters later"]
...
| `scripts/test-integration.sh` | Starts the `mariadb` service from `deploy/compose`, waits for it, then runs `scripts/test.sh` with `REQUIRE_INTEGRATION=1`. | |
| `Makefile` | build / test / test-integration / test-race / test-asan / conformance / lint / generate / docker / clean. `build` delegates to `scripts/build.sh`; `test` and sanitizer variants delegate to `scripts/test.sh`; `test-integration` starts Compose MariaDB first; `conformance` delegates to `scripts/conformance.sh`; `generate` is now only local formatting because spec artifact generation moved to `iiif-spec`. | |
...
- **Conformance smoke is not exhaustive.** CI now checks a running compose
  service and calls `iiif-validate.py` when present, but full IIIF validator
  matrices should live in `iiif-spec` and be expanded there.
- **File-cache eviction is filesystem-scanned.** It now uses standard sorting
  over candidate entries, which is fine for practical cache sizes; very large
  deployments may eventually want an indexed LRU.
