...
`make test-integration` starts the `deploy/compose` stack, using
`ghcr.io/libops/triplet:${GIT_BRANCH:-main}` unless `TRIPLET_IMAGE` is set,
then runs the Docker-backed test suite with `TEST_DSN` plus the HTTP
conformance smoke checks against a generated `sample.ppm` fixture; `make test`
remains deterministic and skips DB integration tests when no database is
available.
