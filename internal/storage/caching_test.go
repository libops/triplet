...
func (o *countingOpener) Open(ctx context.Context, _ string) (io.ReadSeekCloser, Meta, error) {
	o.mu.Lock()
	o.calls++
	o.mu.Unlock()
	if o.blockCh != nil {
...
func TestCachingOpenerDedupesConcurrentStaleRefresh(t *testing.T) {
	store := newTestCacheStore([]byte("old"), cache.Entry{
		ContentType: "image/png",
		Size:        3,
		StoredAt:    time.Now().Add(-2 * time.Hour),
	})
	inner := &countingOpener{body: []byte("new"), blockCh: make(chan struct{})}
