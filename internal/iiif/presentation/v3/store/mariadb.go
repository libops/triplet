...
const (
	createManifestsTableSQL = `CREATE TABLE IF NOT EXISTS iiif_presentation_manifests (
	...
	createAnnotationPagesTableSQL = `CREATE TABLE IF NOT EXISTS iiif_presentation_annotation_pages (
	  item_id varchar(255) NOT NULL,
  ...
  created_at timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),
	...
	selectManifestSQL = `SELECT body FROM iiif_presentation_manifests WHERE item_id = ?`

	selectAnnotationPageSQL = `SELECT body FROM iiif_presentation_annotation_pages WHERE item_id = ? AND canvas_id = ?`

	upsertAnnotationPageSQL = `INSERT INTO iiif_presentation_annotation_pages (item_id, canvas_id, body)
	VALUES (?, ?, ?)
