...
func Parse(rest string) (Request, error) {
	...
	switch len(parts) {
	case 1:
		id, err := decodeIdentifier(parts[0])
		if err != nil {
			return Request{}, err
		}
	...
	default:
		return Request{}, fmt.Errorf("%w: expected 1, 2, or 5 path segments, got %d", ErrSyntax, len(parts))
	}
}
...
func parseImage(parts []string) (Request, error) {
	...
	rot, err := parseRotation(parts[3])
	if err != nil {
		return Request{}, err
	}
	dot := strings.LastIndexByte(parts[4], '.')
	if dot < 1 || dot == len(parts[4])-1 {
		return Request{}, fmt.Errorf("%w: expected {quality}.{format}, got %q", ErrSyntax, parts[4])
	}
	quality, err := parseQuality(parts[4][:dot])
	if err != nil {
		return Request{}, err
	}
	format, err := parseFormat(parts[4][dot+1:])
	if err != nil {
		return Request{}, err
	}
	return Request{
