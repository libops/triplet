...
func (p *Pipeline) Transform(ctx context.Context, req parse.Request, w io.Writer) (Result, error) {
	...
	if outW != regionW || outH != regionH {
		...
		opts := vg.DefaultResizeOptions()
		opts.Vscale = scaleY
		if err := img.Resize(scaleX, opts); err != nil {
...
func encode(img *vg.Image, target *vg.Target, w io.Writer, format parse.Format) (string, error) {
	...
	case parse.FormatJPG:
		if err := img.JpegsaveTarget(target, jpegSaveOptions()); err != nil {
...
func savePDF(img *vg.Image, w io.Writer) error {
	...
	if err := img.JpegsaveTarget(jpegTarget, jpegSaveOptions()); err != nil {
		jpegTarget.Close()
...
func jpegSaveOptions() *vg.JpegsaveTargetOptions {
	opts := vg.DefaultJpegsaveTargetOptions()
	opts.OptimizeCoding = true
	opts.SubsampleMode = vg.SubsampleOn
	opts.Keep = vg.KeepNone
	return opts
}
