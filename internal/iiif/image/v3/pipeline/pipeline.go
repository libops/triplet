...
func (p *Pipeline) Transform(ctx context.Context, req parse.Request, w io.Writer) (Result, error) {
	...
	contentType, err := encode(img, target, req.Format)
	if err != nil {
		return Result{}, err
	}
...
func encode(img *vg.Image, target *vg.Target, format parse.Format) (string, error) {
	switch format {
	...
	case parse.FormatJP2, parse.FormatPDF:
		return "", fmt.Errorf("format %q not implemented in spike", format)
	}
