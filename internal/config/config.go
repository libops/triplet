...
type Image struct {
	...
	JPEGShrinkOnLoad   bool   `yaml:"jpeg_shrink_on_load"`
...
func (c *Config) applyDefaults() {
	...
	if c.IIIF.Image.ColorManagement == "" {
		c.IIIF.Image.ColorManagement = "normalize"
	}
	if c.IIIF.Image.LoadAccess == "" {
		c.IIIF.Image.LoadAccess = "auto"
	}
	c.IIIF.Image.JPEGShrinkOnLoad = true
	c.IIIF.Image.InfoDimensionCache = true
...
func (c *Config) validate() error {
	...
	case "normalize", "none":
	default:
		return fmt.Errorf("iiif.image.color_management: %q not one of normalize|none", c.IIIF.Image.ColorManagement)
	}
